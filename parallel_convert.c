...
//WORK IN PROGRESS
void createHtml(char *filename){ //dont forget to close file //filename is the name of the converted picture
  FILE * html; //<meta http-equiv='refresh' content="2;URL=file:///$absolute_filepath_output_folder/$next_page.html">
  //char metaTag[] = '<meta http-equiv="refresh" content="2;URL=';//file:///$absolute_filepath_output_folder/$next_page.html">'; //consider creating 2 functions; 1 to create and 1 to refresh //note: append location of html page you want to link to.
  char formatName[BUF_SIZE];
  //	<img src="./output_directory//thumbs/ladyslippers_thumb.jpg"/></a>
  char imgSrc[] = '<img src="./output_directory//thumbs/ladyslippers_thumb.jpg"/></a>'; //to do : specify the output directory - need the name of the output
  sprintf(formatName,"image%ld", (long) getpid()); //append process ID to name to identify what kind of html page it'll be ie; {gif, png, or bmp}
  strcat(formatName,".hmtl");
  html = fopen(formatName, "a"); //open formatted name in append mode
  //fprintf(html, "%s\n", );
  fclose(html);
}
