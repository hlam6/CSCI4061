/******************************************************************************
 * Notes:
 *  * open a DIR* and an output file descriptor (with open), then fork the
 *    program.
 *      * Nevermind, we don't want the process to read from the same DIR*. Same
 *        file should be okay though. 
 *
*///***************************************************************************
...
ImageType get_type(char *filename); 
bool is_hidden(char *fname); 
ImageFile [] getInputImages(char *path); 
...
ImageType get_type(char *filename) { 
    ...
    if (file_length > 4 && *(filename += file_length - 4) == '.') {
        switch (*++filename) {
            case 'j':
                if (*++filename == 'p' && *++filename == 'g')
                    return jpeg;
                break;
            case 'p':
                if (*++filename == 'n' && *++filename == 'g') 
                    return png;
                break; 
            case 'b': 
                if (*++filename == 'm' && *++filename == 'p')
                    return bmp;
                break; 
            default: break;
        }
...
ImageFile [] getInputImages(char *path) { 
    ...
    if (dirStream != NULL) { 
        while (entry = readdir(dirStream)) {
            if (stat(entry, &sbuf) != -1 && S_ISREG(sbuf.st_mode)) { 
                if ((type = get_type(entry->d_name)) != NaI) { 
                   // strcpy(
                }
            } 
        }
